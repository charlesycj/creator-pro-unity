...
public class ObjectDown : MonoBehaviour
{
    ...
    public void SpawnObject()
    {
        ...
        Vector3 spawnPosition = new Vector3(randomX, 10f, 0f); // Y는 10으로 설정 (위에서 내려오기 시작)

        // 오브젝트 생성
        GameObject newObject = Instantiate(objectPrefab, spawnPosition, Quaternion.identity);

        // 생성된 오브젝트에 "Down" 스크립트 추가하고 속도 설정
        Down DownScript = newObject.AddComponent<Down>();
        DownScript.speed = initialSpeed; // 초기 속도 전달
    }
}

public class Down : MonoBehaviour
{
    public float speed;               // ObjectDown에서 전달받는 초기 속도 및 증가 속도 
    void Update()
    {
